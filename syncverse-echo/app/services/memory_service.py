...
class MemoryService:
    ...
    def semantic_search(
        ...
        try:
            ...
            # Every other integration point in this service degrades
            # gracefully on failure (create_memory still persists to
            # Postgres if embedding fails). This call previously didn't:
            # a Chroma/embedding error here propagated uncaught through
            # EchoService.chat() and turned into a hard 500 on POST
            # /chat, instead of Echo simply answering without retrieved
            # memory context like it already does when a project
            # legitimately has none.
            logger.error(
                "Vector store query failed for project=%s, degrading to no "
                "retrieved context: %s", project_id, exc,
            )
            return []
