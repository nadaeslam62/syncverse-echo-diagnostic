...
class VectorStoreService:
    ...
    def count(self, project_id: uuid.UUID) -> int:
        """Vectors indexed for this project. Used by the diagnostics
        endpoint to detect drift between Postgres (source of truth) and
        Chroma (semantic index) - e.g. rows exist in Postgres but
        embedding never succeeded, or the reverse after a partial
        restore."""
        return self._get_collection(project_id).count()
